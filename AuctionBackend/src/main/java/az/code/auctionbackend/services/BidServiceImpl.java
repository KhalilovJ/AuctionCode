...
//        bidController.sendUpdates(bidOut);
    ...
    public BidDto makeBid(String username, Long lotId, double bidValue){
        ...
        if (redisLot != null && redisLot.getEndDate().isAfter(LocalDateTime.now())
                && redisLot.getStartDate().isBefore(LocalDateTime.now()) && redisLot.getStatus() == 1){ // if it's null lot is closed and we can't make bids
...
//        Thread.sleep(500);
//        makeBid("malishov", 15502l, 600);
//        makeBid("Khalil", 15502l, 700);
//        makeBid("admin6", 15502l, 800);
//        makeBid("user8", 15502l, 950);
//
//    }
